...
Endpoints:
    ...
    POST /validate      -> analyse a PPTX and return summary + issues

Request JSON for /validate:
    { "fileName": "...", "region": "AMER|EUR|MENA", "fileContent": "<base64 pptx>" }

Deployed on Vercel using ZERO-CONFIG detection:
    - This file (app.py) is at the REPO ROOT.
    - There is NO vercel.json and NO api/ folder.
    - Vercel auto-detects Flask and loads the top-level `app`, preserving
      the real request path so /health and /validate route correctly.
A catch-all route is included as a safety net so unknown paths return JSON.
"""
...
# Primary routes
app.add_url_rule("/validate", "validate", _do_validate, methods=["POST"])
app.add_url_rule("/health", "health", _do_health, methods=["GET"])

# Safety-net aliases (in case Vercel adds an /api or /api/index prefix)
app.add_url_rule("/api/validate", "validate_api", _do_validate, methods=["POST"])
app.add_url_rule("/api/index/validate", "validate_api_index", _do_validate, methods=["POST"])
app.add_url_rule("/api/health", "health_api", _do_health, methods=["GET"])
app.add_url_rule("/api/index/health", "health_api_index", _do_health, methods=["GET"])
...
def home():
    return jsonify({"service": "PPT Content Quality Validator",
                    "status": "running",
                    "endpoints": ["/health (GET)", "/validate (POST)"]}), 200


# Catch-all: any unmatched path returns helpful JSON instead of an HTML 404.
@app.route("/<path:_ignored>", methods=["GET", "POST"])
def catch_all(_ignored):
    # If someone POSTs to an unknown path, still try to validate (robust for flows).
    if request.method == "POST":
        return _do_validate()
    return jsonify({"service": "PPT Content Quality Validator",
                    "status": "running",
                    "hint": "POST to /validate, GET /health",
                    "path_seen": request.path}), 200
